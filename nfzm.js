...
var body = $response.body.replace(/is_free": \d/g, 'is_free": 1').replace(/"price": \d+/g, '"price": 0').replace(/"vip_price": \d+/g, '"vip_price": 0').replace(/"isdigg": \w+/g, '"isdigg": true').replace(/"isfav":"\w+"/g, '"isfav":"true"').replace(/"isbuy":\w+/g, '"isbuy":true').replace(/"member_type":"\d"/g, '"member_type":"1"').replace(/"expire_time":"\w+"/g, '"expire_time":"2022-05-05 11:11:11"').replace(/cnt_mode":"\w+"/g, 'cnt_mode":"free"').replace(/cnt_comment_count":"\d"/g, 'cnt_comment_count":"2"');
$done({ body });
